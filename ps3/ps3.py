...
def is_valid_word(word, hand, word_list):
    ...
    word = word.lower()
    boolean_in_wordlist = True
    boolean_in_hand = True
    boolean = True
    updated_hand = hand.copy()
    location = word.find('*')
    if location == -1:
        ...
        for vowel in VOWELS:
            ...
            if word in word_list:
                ...
                for letter in word:
                    if letter in hand:
                        updated_hand[letter] -= 1
                    elif word.find(letter) == location:
                        boolean *= True
                    else:
                        boolean = False
                    # if updated_hand.get(letter, 0) < 0:
                    #     boolean_in_hand = False
            else:
                boolean = False
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while(True):
        ...
        # if input is two exclamation points
        if word == '!!':
...
#     play_hand(hand, word_list=word_list)
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    num_hands = int(input('Please give us the total number of hands. '))
    total_score = 0
    iteration = 0
    current_hand = deal_hand(HAND_SIZE)
    print('Current hand: ', end='')
    display_hand(current_hand)
    substitute_bool = str(input('Would you like to substitute a letter? ')).lower()
    if substitute_bool == 'yes':
        substitute_letter = str(input('Which letter would you like to replace: ')).lower()
        total_score += play_hand(substitute_hand(current_hand,substitute_letter),word_list)
    else:
        total_score += play_hand(current_hand, word_list)

    while(iteration<num_hands-1):
        replay_bool = str(input('Would you like to replay the hand? ')).lower()
        if replay_bool == 'no':
            current_hand = deal_hand(HAND_SIZE)
        elif replay_bool == 'yes':
            iteration += 1
        else:
            print('That\'s not a valid input. Try with yes or no.')
            continue
        print('Current hand: ', end='')
        display_hand(current_hand)
        substitute_bool = str(input('Would you like to substitute a letter? ')).lower()
